...
**Last updated:** 2026-09-27 (Phase 4 code + DB done — `word_status` `'new'` migration applied live; word/progress hooks read real `words`/`word_progress`/`study_sessions` via the Clerk-bound client with a mock fallback; grade/favorite/study-session mutations upsert under RLS, validated with simulated Clerk JWT claims; Study/Home/Browse/Progress/Profile rewired and weekly labels moved from minutes to words. Remaining: on-device sign-in to confirm the empty first-run experience and close the exit criterion — that login also closes Phase 3.)