...
**Steps (device — a development build, not Expo Go):** enable the reminder in Settings → confirm the OS permission prompt → advance the clock past `reminder_time`.
**Expected:** toggle-on triggers the permission prompt once; denial keeps the switch off and shows the denied hint (it can never render "enabled" while the OS blocks it); the DAILY trigger `daily-study-reminder` is scheduled; a fired notification deep-links to Home. Offline: cut and restore connectivity in the simulator (or `simulateOffline`) — the offline screen/banner appears, and previously failed React Query queries auto-refetch on reconnect via the NetInfo→`onlineManager` bridge (never a stale screen after recovery). In Expo Go the reminder is a safe no-op, never a crash.
**Status:** 🔶 code verified 2026-09-27 — `src/features/notifications/api.ts` (handler, channel, permission helpers, fixed-identifier DAILY schedule/cancel) typechecks and lints clean; `onlineManager.setEventListener`→NetInfo wired at the module scope of `_layout.tsx`. **2026-09-27 fix:** `expo-notifications` can't be *imported* in Expo Go (removed SDK 53 — it throws at module scope and previously crashed the entire route tree). The module is now lazy-loaded behind an `expo-constants` execution-environment guard (`StoreClient` → null), all functions no-op safely, and the tap-to-Home deep-link uses `addNotificationResponseReceivedListener`; Expo Go runs normally. On-device firing + network-cut confirmation deferred — now requires a dev build.
...
- **2026-09-27 (Expo Go crash fix):** running in Expo Go crashed the whole app — `import * as Notifications from 'expo-notifications'` throws at module scope because the native module was removed from Expo Go in SDK 53 (reported as `Android Push notifications … was removed from Expo Go`). Fix: `loadNotifications()` lazy dynamic import guarded by `Constants.executionEnvironment !== StoreClient` (Expo Go) + `Platform.OS !== 'web'`; every notifications API becomes a safe no-op in Expo Go; the reminder deep-link swaps `useLastNotificationResponse()` for `addNotificationResponseReceivedListener`; Settings denied-hint copy explains the Expo Go/dev-build split. App boots normally in Expo Go; reminders require a development build. `tsc --noEmit`, `expo lint`, `expo-doctor` 21/21 clean.